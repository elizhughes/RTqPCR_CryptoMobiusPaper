...
colkey <- create_colkey_6_in_24(target_id = c("BLP1", "MEU1", "MEP1", "SRP14", "ACT1", "GDP1"))
...
```{r plate_sep_refgenes_plate2}
  ...
  ggplot(data = filter(plate2_sep, target_id == c("SRP14", "ACT1", "GDP1") )) +
  geom_point(aes(x = target_id, y = cq, shape = prep_type, colour = target_id),
    position = position_jitter(width = 0.2, height = 0)
  ) +
  labs(
...
plot2 <- (data = filter(plate2_norm, target_id != c("SRP14", "ACT1", "GDP1"), Sample != c("H99" )))
```
